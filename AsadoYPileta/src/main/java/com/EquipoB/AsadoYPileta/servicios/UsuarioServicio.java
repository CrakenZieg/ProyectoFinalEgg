...
public class UsuarioServicio implements UserDetailsService {
    ...
    @Transactional
    public void cambiarPassword(String email,String password) throws MiException{
        
        Usuario usuario = usuarioRepositorio.buscarPorEmail(email);
       
        // BCryptPasswordEncoder es una clase que se utiliza comúnmente para realizar hashing y verificación de contraseñas seguras.
        if(usuario != null && new BCryptPasswordEncoder().matches(password, usuario.getPassword())){  //se procede a verificar si la contraseña proporcionada coincide con la contraseña almacenada en la base de datos. Esto se hace utilizando el método matches 
           
            usuario.setPassword(new BCryptPasswordEncoder().encode(password)); // Si la contraseña antigua coincide, esta línea establece la nueva contraseña proporcionada por el usuario después de codificarla con BCryptPasswordEncoder. Esto garantiza que la contraseña se almacene de forma segura en la base de datos.
           //encode: proporciona en la forma que se va a ingresar la contraseña, en este caso String
            usuarioRepositorio.save(usuario);
